...
static const std::string TRANSFORMER_NAME = "gnome-shell-overview";
...
class activities_view_t
{
  public:
    ...
    wf::geometry_t get_screen_preview_geometry() const { return screen_preview_geometry; }
    wf::geometry_t get_current_desktop_geometry() const { return desktop_preview_anim.current(); }
    const std::vector<wf::geometry_t>& get_workspace_geometries() const { return workspace_preview_geometries; }
    int get_current_workspace_index() const { return current_workspace.y * workspace_cols + current_workspace.x; }
                                   ...
                                   wf::scene::damage_callback push_damage)
        {
            ...
            for (int y = 0; y < wsize.height; y++)
            {
                for (int x = 0; x < wsize.width; x++)
                {
                    ...
                    auto push_damage_child = [this, self, idx = workspace_captures.size()] (const wf::region_t& damage)
                    {
...
class gnome_shell_output_t : public wf::per_output_plugin_instance_t
{
  public:
    ...
    void init() override
    {
        ...
        LOGI("GNOME Shell output initialized");
    }
    ...
    void render_panel_overlay()
    {
        ...
        if (panel_tex)
        {
            ...
            wf::geometry_t panel_geom = {og.x, og.y + og.height - panel->panel_height, panel->panel_width, panel->panel_height};
...
class wayfire_gnome_shell_t : public wf::plugin_interface_t
{
  public:
    wf::option_wrapper_t<int> opt_panel_height{"gnome-shell/panel_height"};
    wf::option_wrapper_t<std::string> opt_panel_color{"gnome-shell/panel_color"};
    wf::option_wrapper_t<int> opt_corner_radius{"gnome-shell/corner_radius"};
    wf::option_wrapper_t<int> opt_animation_duration{"gnome-shell/animation_duration"};
    ...
    wf::signal::connection_t<wf::post_input_event_signal<wlr_pointer_button_event>>
        on_button = [this](wf::post_input_event_signal<wlr_pointer_button_event> *ev) { handle_button(ev->event); };
    ...
    void init() override
    {
        ...
        LOGI("GNOME Shell plugin initialized");
    }
    ...
    void fini() override
    {
        ...
        outputs.clear();
        LOGI("GNOME Shell plugin finalized");
    }

    void add_output(wf::output_t *output)
    {
        auto inst = std::make_unique<gnome_shell_output_t>();
...
DECLARE_WAYFIRE_PLUGIN(gnome_shell::wayfire_gnome_shell_t);
